...
import type { PointPrevisionnel } from "@/lib/tresorerie-previsionnelle";
...
/** Même style que TresorerieChart (historique) mais en sauge, pour distinguer visuellement un
 *  passé constaté d'une projection à hypothèses. */
export default function TresoreriePrevisionnelleChart({ points }: { points: PointPrevisionnel[] }) {
  ...
  return (
    <>
      <svg viewBox={`0 0 ${LARGEUR} 155`} width="100%" height="150">
        ...
        <path d={aire} fill="#5C7A5B" opacity="0.1" />
        <polyline points={polyline} fill="none" stroke="#5C7A5B" strokeWidth="2" strokeDasharray="4 3" />
        <g fill="#5C7A5B">
          {coords.map((c) => (
      ...
      <div className="chart-caption">
        De {formatEuros(premier.soldeCents)} ({premier.label}) à {formatEuros(dernier.soldeCents)} ({dernier.label}) — projection à partir du solde actuel, hypothèses détaillées ci-dessous
      </div>
    </>
  );
}
